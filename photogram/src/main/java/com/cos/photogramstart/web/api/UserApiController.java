...
public class UserApiController {
	...
	// user 1번이 subscribe하고 있는 모든 사람 보여줌
	public ResponseEntity<?> subscribeList(@PathVariable int pageUserId
			,@AuthenticationPrincipal PrincipalDetails principalDetails){
		List<SubscribeDto> subscribeDto = subscribeService.구독리스트(principalDetails.getUser().getId(),pageUserId); //여기다가 정보 담고 리턴하기
	...
	public CMRespDto<?> update(@PathVariable int id
		...
		if(bindingResult.hasErrors()) {
			...
			//응답시에 userEntity의 모든 getter함수가 호출되어 JSON으로 파싱되어 응답함
		}
	}
}
