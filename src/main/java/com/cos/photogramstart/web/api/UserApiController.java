...
public class UserApiController {
	...
	public ResponseEntity<?> profileImageUrlUpdate(@PathVariable int principalId,MultipartFile profileImageFile
		...
		//js에서 넘어온 data:formData를 => MultipartFile profileImageFile(html input file name이 profileImageFile)로 받음 
		User userEntity = userService.회원프로필사진변경(principalId,profileImageFile);
		/* 프로필 사진 변경 후 , 세션 값이 바껴야 된다.*/
	...
	public CMRespDto<?> update(@Valid UserUpdateDto userUpdateDto,
		...
		//System.out.println(userUpdateDto);
			...
			principalDetails.setUser(userEntity);//세션에도 반영될수있게
			return new CMRespDto<>(1,"회원수정 완료",userEntity); 
			//userEntity를 응답시에 userEntity의 모든 getter함수가 호출되고 JSON으로 파싱되어 응답한다.
	}
}
